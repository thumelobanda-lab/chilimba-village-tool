...
export default function registerGroupRoutes(router) {
  ...
  // field and generates the group's code itself (never admin-supplied —
  // see generateUniqueGroupCode in auth.js); the new admin is the only
  // account this creates, so it can't be used to tamper with an existing
  // group.
  router.post("/api/groups", async ({ request, env, cors }) => {
    const body = await request.json();
    if (!body.groupName || !body.adminName || !body.pin) {
      throw new HttpError(400, "groupName, adminName, and pin are all required.");
    }
